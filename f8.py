...
FAILURE_ROUNDS = [2, 3, 4, 5, 6, 7, 8, 9]  # 8 falhas (80%) - CENÁRIO EXTREMO!
...
def main():
    print("🚀 INICIANDO COMPARAÇÃO DE ESTRATÉGIAS - 8 FALHAS")
    ...
    print("💀 CENÁRIO EXTREMO: 80% das rodadas com falhas - Teste de stress máximo!")
    ...
    print(f"\n{'='*70}")
    print("📈 RESUMO COMPARATIVO - 8 FALHAS (80%) - CENÁRIO EXTREMO")
    print(f"{'='*70}")
    ...
    for strategy_name, history in results.items():
        ...
        if successful_count > 0:
            efficiency = successful_count / total_rounds
            if efficiency >= 0.9:
                print(f"   • 🎯 DESEMPENHO: EXCELENTE (>{efficiency:.0%} eficiência)")
            elif efficiency >= 0.7:
                print(f"   • ⚠️ DESEMPENHO: BOM (>{efficiency:.0%} eficiência)")
            else:
                print(f"   • ❌ DESEMPENHO: CRÍTICO (>{efficiency:.0%} eficiência)")
